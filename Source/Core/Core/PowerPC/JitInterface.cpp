...
void GetProfileResults(ProfileStats* prof_stats)
{
  ...
  jit->GetBlockCache()->RunOnBlocks([&prof_stats](const JitBlock& block) {
...
int GetHostCode(u32* address, const u8** code, u32* code_size)
{
  ...
  JitBlock* block = jit->GetBlockCache()->GetBlockFromStartAddress(*address, MSR);
  if (!block)
  {
    for (int i = 0; i < 500; i++)
    {
      block = jit->GetBlockCache()->GetBlockFromStartAddress(*address - 4 * i, MSR);
      if (block)
        break;
    }
...
void CompileExceptionCheck(ExceptionType type)
{
  ...
  case ExceptionType::EXCEPTIONS_PAIRED_QUANTIZE:
    ...
    break;
  case ExceptionType::SPECULATIVE_CONSTANTS:
