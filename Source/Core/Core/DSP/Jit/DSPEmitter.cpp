...
{
  const u8 flags = DSPAnalyzer::GetCodeFlags(m_compile_pc);
...
void DSPEmitter::Compile(u16 start_addr)
{
  ...
  */
  ...
  {
    if (DSPAnalyzer::GetCodeFlags(m_compile_pc) & DSPAnalyzer::CODE_CHECK_INT)
    ...
    // by the analyzer.
    if (DSPAnalyzer::GetCodeFlags(static_cast<u16>(m_compile_pc - 1u)) & DSPAnalyzer::CODE_LOOP_END)
    {
    ...
    if (DSPAnalyzer::GetCodeFlags(m_compile_pc) & DSPAnalyzer::CODE_IDLE_SKIP)
    {
      break;
    }
  }
