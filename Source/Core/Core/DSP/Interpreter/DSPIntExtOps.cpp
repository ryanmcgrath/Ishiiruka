...
void applyWriteBackLog()
{
  ...
  for (int i = 0; writeBackLogIdx[i] != -1; i++)
  {
    ...
    value |= DSPInterpreter::dsp_op_read_reg(writeBackLogIdx[i]);
#endif
    DSPInterpreter::dsp_op_write_reg(writeBackLogIdx[i], value);
...
void zeroWriteBackLog()
{
  ...
  for (int i = 0; writeBackLogIdx[i] != -1; i++)
  {
    DSPInterpreter::dsp_op_write_reg(writeBackLogIdx[i], 0);
  }
#endif
}
